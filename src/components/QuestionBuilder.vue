...
const { handleSubmit, setFieldValue, resetForm } = useForm({
  ...
  initialValues: {
    question: '',
    answers: [{ answer: '', optional_answer: '' }],
    points: '',
    duration: '',
    files: [],
    type: ''
  ...
  <form @submit="onSubmit">
    <div class="flex items-center justify-between w-full">
      <div class="flex items-center gap-x-4">
        ...
        <FormField name="duration" v-slot="{ componentField }">
          <div class="flex items-start flex-col">
            <MenuSelect
              v-bind="componentField"
              v-model="selectedQuestion.duration"
              :options="duration"
      ...
      >
        <Icon
          ...
          class="text-[25px] text-[#D92D20] cursor-pointer"
        />
      </div>
    </div>

    <div
      class="p-[40px] border border-[#DBDCDE] rounded-[4px] flex items-start justify-start flex-col my-6"
    >
      <div class="w-full">
      ...
      <div class="w-full my-6">
        ...
        <FormField name="files" v-slot="{ componentField, errors }">
          <FileUploader
            @share-files="(e) => setFieldValue('files', e)"
            :class="[
              Object.keys(errors).length > 0 ? 'border-2 border-red-600 hover:bg-red-200' : null
            ]"
            v-bind="componentField"
            v-model="selectedQuestion.files"
            ref="fileSelector"
          />
          <FormMessage />
          <div class="mt-4 w-full">
            <template v-for="(file, index) in selectedQuestion.files">
              <File icon="radix-icons:file" :index="index" :file="file" />
            </template>
            <Button
              @click="openFileSelector"
              v-if="selectedQuestion.files.length > 0"
              variant="outline"
              class="h-[44px] text-[#00AD1B] my-3"
              >Add upload</Button
            >
          </div>
        </FormField>
          ...
          <FormField name="answers" v-slot="{ componentField, errors }">
            <template v-for="answer in selectedQuestion.answers">
              <div class="flex items-center gap-x-3">
                ...
                <div
                  class="w-[40px] h-[40px] border border-[#DBDCDE] flex items-center justify-center"
                >
                  <Icon
                    icon="solar:trash-bin-minimalistic-linear"
              ...
              <div class="w-full flex items-start flex-col my-4">
                <Label class="mb-px text-[#667085]" for="question"
    ...
    <div class="border-t border-b border-[#DBDCDE]">
      <div class="flex items-center justify-between my-4">
        <Button type="submit" variant="outline" class="h-[44px] text-[#00AD1B] my-3"
